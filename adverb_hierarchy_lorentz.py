...
class AdverbHierarchyProjection(nn.Module):
    """
    3-level hierarchy: Adverb ⊃ Pair ⊃ Video
    ...
    def forward(self, batch):
        ...
        # === CONTRASTIVE LOSSES ===
        # Level 1-2: Adverb ↔ Pair
        adverb2pair = -L.pairwise_dist(adverb_hyp, pair_hyp, _curv)
        pair2adverb = -L.pairwise_dist(pair_hyp, adverb_hyp, _curv)
        ...
        # Level 1-3: Adverb ↔ Video (auxiliary)
        adverb2video = -L.pairwise_dist(adverb_hyp, video_hyp, _curv)
        video2adverb = -L.pairwise_dist(video_hyp, adverb_hyp, _curv)

        contrastive_loss = (
            nn.functional.cross_entropy(adverb2pair * _scale, labels) +
            nn.functional.cross_entropy(pair2adverb * _scale, labels) +
            nn.functional.cross_entropy(pair2video * _scale, labels) +
            nn.functional.cross_entropy(video2pair * _scale, labels) +
            nn.functional.cross_entropy(adverb2video * _scale, labels) +
            nn.functional.cross_entropy(video2adverb * _scale, labels)
        ) / 6

        # === 3-LEVEL ENTAILMENT LOSSES ===
        # Level 1 ⊃ Level 2: Adverb ⊃ Pair
        entail_adverb_pair = self.entailment_loss(adverb_hyp, pair_hyp)

        # Level 2 ⊃ Level 3: Pair ⊃ Video
        entail_pair_video = self.entailment_loss(pair_hyp, video_hyp)

        # Level 1 ⊃ Level 3: Adverb ⊃ Video (transitive, weighted 0.5)
        entail_adverb_video = 0.5 * self.entailment_loss(adverb_hyp, video_hyp)
        ...
        return {
            ...
            "logging": {
                ...
                "entail_adverb_pair": entail_adverb_pair,
                "entail_pair_video": entail_pair_video,
                "entail_adverb_video": entail_adverb_video,
                "logit_scale": _scale,
                "curv": _curv,
                "adverb2pair_logits": adverb2pair,
                "pair2video_logits": pair2video,
                "adverb2video_logits": adverb2video,
            },
        }
...
class CombinedModel(nn.Module):
    def __init__(self, config):
        ...
        self.projection_module = AdverbHierarchyProjection(config=config)
        self.config = config
...
class Trainer:
    ...
    def train_one_epoch(self):
        ...
        for batch_idx, batch_dict in enumerate(pbar):
            ...
            with torch.no_grad():
                adv2p_logits = outputs['logging']['adverb2pair_logits']
                p2v_logits = outputs['logging']['pair2video_logits']
                in_batch_labels = torch.arange(a2p_logits.size(0), device=a2p_logits.device)
                action_batch_acc = (adv2p_logits.argmax(dim=1) == in_batch_labels).float().mean().item()
            ...
            if self.wandb_enabled and batch_idx % 10 == 0:
                wandb.log({
                    ...
                    'step/entail_adverb_pair': outputs['logging']['entail_adverb_pair'].item(),
                    'step/entail_pair_video': outputs['logging']['entail_pair_video'].item(),
                    'step/entail_adverb_video': outputs['logging']['entail_adverb_video'].item(),
                    ...
                    'step/adverb2pair_top1_inbatch': adverb_batch_acc,
        ...
        if self.wandb_enabled:
            ...
            wandb.log({
                ...
                'train/adverb2pair_top1_inbatch': avg_adverb_acc,
        ...
        print('E: %d | L: %.2E | L_c: %.2E | L_e: %.2E | Adv@1: %.3f | Pair@1: %.3f | Grad: %.2E | LR: %.2E' %
              (self.current_epoch, avg_loss, avg_contrastive_loss, avg_entail_loss,
               avg_adverb_acc, avg_pair_acc, self.grad_norm.avg, self.optimizer.param_groups[0]['lr']))
    ...
    def validate(self):
        ...
        with torch.no_grad():
            # Build adverb gallery
            adverb_indices = torch.arange(self.num_adverbs, device=self.device)
            adverb_raw = self.model.aa_model.get_adverb_embeddings(adverb_indices)
        ...
        print('  [dist]  Adverb Top-1: %.4f Top-5: %.4f | Pair Top-1: %.4f Top-5: %.4f' % (d_adv1, d_adv5, d_pair1, d_pair5))
        ...
        if self.wandb_enabled:
            wandb.log({
                ...
                'val/adverb_top1': d_adv1,
                'val/adverb_top5': d_adv5,
                'val/pair_top1': d_pair1,
                'val/pair_top5': d_pair5,
                'val/compositional_accuracy': compositional_acc,
                'val/norm_adverb_gallery': wandb.Histogram(embedding_norm(adverb_gallery).cpu().numpy()),
    ...
    def train(self, num_epochs):
        ...
        for epoch in range(self.current_epoch, num_epochs):
            ...
            print(f'  Best Adverb Acc: {self.best_adverb_acc:.4f} | Best Pair Acc: {self.best_pair_acc:.4f}')
        ...
        if self.wandb_enabled and filename == 'best_model.pth':
            artifact = wandb.Artifact(
                ...
                description=f'Best model at epoch {self.current_epoch} with val_loss={self.best_val_loss:.4f}, '
                            f'adverb_acc={self.best_adverb_acc:.4f}, pair_acc={self.best_pair_acc:.4f}, '
                            f'compositional_acc={self.best_compositional_acc:.4f}',
                metadata={
                    'epoch': self.current_epoch,
                    'val_loss': self.best_val_loss,
                    'action_acc': self.best_adverb_acc,
...
def main():
    parser = argparse.ArgumentParser(description='Hyperbolic Action Hierarchy Training (Adverb → Pair → Video)')
    ...
    parser.add_argument('--wandb-project', type=str, default='hyperbolic-adverb-hierarchy')
